    ...
    for key, samples in generated_data.items():
        ...
        try:
            ...
            if len(output_labels) >= 5:
                ...
                # Not enough samples for bootstrapping
                lang_std_devs = None
...
def task2_metrics(generated_data):
    ...
    try:
        ...
        # Keep existing code for high_overlap_samples and low_overlap_samples files
...
def generate_example_prompts_file(generated_data, templates, demonstrations, filenames):
    """
    Generate a TSV file with complete example prompts including demonstrations, 
    expected outputs, and generated text for each language combination.
    ...
    try:
        with open('complete_examples_with_demonstrations.tsv', 'w', newline='', encoding='utf-8') as tsv_file:
            fieldnames = [
                'language_combination', 'instruction_language', 'input_language', 
                'raw_instruction', 'raw_input', 'expected_output', 'generated_text', 
                'full_prompt_with_demonstrations', 'selected_demonstrations', 'is_correct', 'overlap_score'
            ]
            writer = csv.DictWriter(tsv_file, fieldnames=fieldnames, delimiter='\t')
            writer.writeheader()
            ...
            for lang_combo, samples in generated_data.items():
                ...
                if good_example:
                    ...
                    instruction_lang_lower = instruction_lang.lower()
                    relevant_demos = select_demonstrations(instruction_lang_lower, demonstrations)
                    
                    # Format demonstrations for readability in TSV
                    demos_formatted = []
                    for demo in relevant_demos:
                        demo_str = f"Instruction: {demo.get('instruction', 'N/A')}\n"
                        
                        # Get the input based on available language keys
                        demo_input = "N/A"
                        for lang in ["en", "it", "pt"]:
                            input_key = f"input_{lang}"
                            if input_key in demo:
                                demo_str += f"Input ({lang}): {demo[input_key]}\n"
                                break
                        
                        demo_str += f"Output: {demo.get('output', 'N/A')}"
                        demos_formatted.append(demo_str)
                    ...
                    writer.writerow({
                        ...
                        'full_prompt_with_demonstrations': full_prompt,
                        'selected_demonstrations': '\n\n'.join(demos_formatted),
        ...
        print(f"Complete examples with demonstrations have been saved to complete_examples_with_demonstrations.tsv")
    except Exception as e:
        print(f"Error saving examples with demonstrations to file: {e}")
...
def main():
    ...
    try:
        ...
        # In the main() function, modify this line:
...
def example_prompts():
    try:
        ...
        # Create file for saving examples
        examples_file = open('interactive_examples_with_demos.txt', 'w', encoding='utf-8')
        
        # Debug print to console and file
        print("First few demonstrations:")
        examples_file.write("First few demonstrations:\n")
        for demo in demonstrations[:3]:
            demo_str = json.dumps(demo, indent=2)
            print(demo_str)
            examples_file.write(f"{demo_str}\n\n")
        
        # Validate inputs
        if not templates:
            msg = "Error: No templates loaded"
            print(msg)
            examples_file.write(f"{msg}\n")
            examples_file.close()
            return "No templates found"
        
        if not demonstrations:
            msg = "Error: No demonstrations loaded"
            print(msg)
            examples_file.write(f"{msg}\n")
            examples_file.close()
        ...
        samples = [
            ...
            {
                "description": "English Instruction with Italian Input",
                "sample": {
                    "instruction": "Identify any idiomatic expressions in the following sentence:",
                    "input_it": "Ieri sera ho deciso di tagliare la testa al toro e affrontare il problema direttamente.",
                    "output": "tagliare la testa al toro"
            ...
            {
                "description": "Italian Instruction with English Input",
                "sample": {
                    "instruction": "Rilevare le espressioni idiomatiche nella seguente frase:",
                    "input_en": "The politician was skating on thin ice with his controversial statements.",
                    "output": "skating on thin ice"
        ...
        for example in samples:
            try:
                ...
                template = select_template(instruction_lang, templates)
                template_info = "Selected Template:\n" + \
                          f"Language: {template.get('lang', 'N/A')}\n" + \
                          f"Prompt Template:\n{template.get('prompt_input', 'N/A')}\n"
                print(template_info)
                examples_file.write(f"{template_info}\n")
                
                # Select demonstrations
                relevant_demos = select_demonstrations(instruction_lang, demonstrations)
                demo_header = f"Selected {len(relevant_demos)} Demonstrations:"
                print(demo_header)
                examples_file.write(f"{demo_header}\n")
                
                for demo in relevant_demos:
                    demo_info = f"- Instruction: {demo.get('instruction', 'N/A')}\n"
                    print(demo_info, end='')
                    examples_file.write(demo_info)
                    
                    for lang in ['en', 'it', 'pt']:
                        input_key = f'input_{lang}'
                        if input_key in demo:
                            input_info = f"  Input ({lang}): {demo[input_key]}\n"
                            print(input_info, end='')
                            examples_file.write(input_info)
                    
                    output_info = f"  Output: {demo.get('output', 'N/A')}\n\n"
                    print(output_info)
                    examples_file.write(output_info)
                ...
                sample_info = "Full Prompt Details:\n" + \
                        f"Instruction: {example['sample'].get('instruction', 'N/A')}\n" + \
                        f"Input: {example['sample'].get('input_en', example['sample'].get('input_it', example['sample'].get('input_pt', 'N/A')))}\n" + \
                        f"Expected Output: {example['sample'].get('output', 'N/A')}\n"
                print(sample_info)
                examples_file.write(f"{sample_info}\n")
                
                full_prompt_header = "\n--- COMPLETE PROMPT ---"
                print(full_prompt_header)
                examples_file.write(f"{full_prompt_header}\n")
                
                print(full_prompt)
                examples_file.write(f"{full_prompt}\n\n")
                ...
                error_msg = f"Error processing example {example['description']}: {e}"
                print(error_msg)
                examples_file.write(f"{error_msg}\n")
                import traceback
                tb = traceback.format_exc()
                print(tb)
                examples_file.write(f"{tb}\n")
        
        examples_file.close()
        print("Examples saved to interactive_examples_with_demos.txt")
        return "Examples generated and saved to file"
    
    except Exception as e:
        error_msg = f"Critical error in example_prompts: {e}"
        print(error_msg)
        import traceback
        tb = traceback.format_exc()
        print(tb)
